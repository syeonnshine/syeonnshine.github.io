...
export default function Hero() {
  return (
    <section className="bg-bg px-6 py-16 sm:px-12 sm:py-24">

      <div className="max-w-3xl mx-auto flex flex-col gap-8 sm:flex-row sm:items-center">
        ...
        <div className="shrink-0">
          <img
            src={profileImg}
            alt="박시연 프로필 사진"
            className="w-40 h-40 sm:w-56 sm:h-56 rounded-full object-cover border border-border"
          />
        </div>
        ...
        <div className="flex-1">
          ...
          <p className="text-lg sm:text-xl text-text-muted leading-relaxed mb-8">
            AG-Grid 기반 대용량 운영툴을 실무에서 만들어 온,<br className="hidden sm:block" />
            기획→구현→운영까지 혼자 굴리는 프론트엔드 개발자
          </p>

          <ul className="flex flex-wrap gap-2 mb-8" aria-label="핵심 기술 스택">
            {heroStacks.map((stack) => (
              <li
                key={stack}
                className="px-3 py-1 text-sm border border-border rounded-md text-accent"
              >
                {stack}
              </li>
            ))}
